...
I found their blogs a bit "markety" - they seem to be fond of coining terms that make relatively simple concepts like "write parameterized functions" sound fancy: "prediction engineering". But perhaps this coining is best interpreted as ML salesmanship? Anyway, I digress. The package had excellent reviews and had some serious engineering to ensure feature generation was scalable.

I found an excellent reticulate-based version of the package called featuretoolsR which gave me a great excuse to give the framework a spin. It was also a good excuse to explore a TidyTuesday dataset that I had my eye on for a long time.... Chopped reviews!
...
1. A time series regression problem: can we predict the next episode's rating based on prior episodes and data about the episode that may be revealed in a trailer?
...
The chopped data consists of a time series (episodes aired on a certain date), episode ratings (the quantity to predict), and a variety of episode data  rich for generating features. 

```{r peek-data, layout="l-body-outset"}
paged_table(chopped_rating)
```

To begin the exploratory analysis, I took a look at the ratings across shows (note I already removed any shows with no rating when I loaded the data):
...
Next, I wanted to take a look at some of the features in the data to continue the exploratory analysis to see if any attributes may have the potential to predict episode rating.

### EDA by Season

One place to start is to see if episode rating was varied by season. Who knew Chopped has 43 seasons?!
...
```
In this case, it appears the ratings decreased gradually overtime, with the exception of the final few seasons which had much fewer episodes with rating data.

### EDA by Judge

I also wanted to look at ratings by judge. I have favorite judges afterall, maybe most people do?
...
1. There are some judges who have only done one episode. This observation makes "judge" a hard variable to use for predictions because there will singular values for certain factors and because never-before-seen values (new judges) will come up in test sets and out of sample predictions.

2. There may be differences between judges who have been on many episodes. Let's take a deeper look:
...
This plot suggests that the "recurring" judges all have ranges that are pretty similar (all the boxplots are overlapping), which means the specifics of a recurring judge may not be that predictive. But what if we compare episodes with recurring judges to episodes with "guest" judges?

```{r guest-judges}
by_judge %>% 
  mutate(type = ifelse(value %in% influential_judges, "recurring", "guest")) %>% 
  ungroup() %>% 
  group_by(type) %>% 
  ggplot(aes(type, episode_rating)) + 
  geom_boxplot() + 
  coord_flip()
```

Again, the boxplots mostly overlap. This makes sense, as the show almost always has a combo of recurring and guest judges. Overall, this suggests to me that the judges on a show might not be that predictive of episode rating. But if we did want to try and use judge as some sort of predictive feature, the most variability appears to exist between recurring judges. 
...
The `_info` fields for each contestant contain free text information about the contestants:
...
The package is designed to help automate feature creation through a process called "deep feature synthesis". In my opinion this title is a like "DAG", it is a name designed to make a pretty simple concept sound fancy. The core idea in featuretools is that you can take primitive operations to create new features,  and then sometimes you can combine those operations to create even more new features. 

For example, in our chopped data set you could take primitives like "week_day" and "mode" to generate a number of new feature columns: `week_day(air_episode)`, `mode(season_rating)`, and then combinations like: `mode(week_day(air_episode))`. 
...
chopped_plus_eda$air_date <- mdy_hms(paste0(chopped_pl$air_date, "20:00:00"))
...
chopped_all %>% 
  ...
  paged_table()
```
Essentially there appeared to be overlap between the beginning of one season and the end of another. This makes sense, because there are 43 seasons over the course of only 11 years. We can look at some of the details:
...
Total out of order is `r out_of_order`. This change makes sense, but it means that in order to avoid "leaking" data about the future, we need to arrange by `air_date` when making test and training splits. 
...
Feature importance for tree models give a sense for which of the many features are making the biggest impact in the model.
...
Looks like our auto-generated features came in handy, along with some of the EDA based work! As expected, the hard work we did on extracting judges didn't help the model much, wheras season and air date did! The model also gleaned something from the ingredient lists and sentiment data, but not much from our hard work extracting city entities!
